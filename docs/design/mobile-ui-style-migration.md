...
- **Nhịp Home:** hàng đầu gồm nhận diện tiệm và thông báo; tiếp theo ngày và tiêu đề `Tổng quan`; bộ chọn `Hôm nay / Tuần này / Tháng này` đứng ngay trên card doanh thu để điều khiển đi trước nội dung nó đổi. Bộ chọn có thanh trượt và đổi nhãn kỳ, số liệu trên card. Sau đó là tối đa hai việc ưu tiên, một CTA chọn hàng, gợi ý có nguồn/kỳ dữ liệu rõ, rồi danh sách bán chạy. Không giữ lưới chức năng lớn, không để chart và nhiều KPI tranh vị trí với hành động chính trên Home; chạm card mở màn phân tích theo kỳ đã chọn.
...
Đặt bộ chọn kỳ `Hôm nay / Tuần này / Tháng này` ngay trên **một thẻ doanh thu bo góc**, theo yêu cầu đặt điều khiển trước nội dung nó điều khiển (thay cho bố cục cũ đặt bộ chọn sau thẻ). `Tuần này` là tuần lịch từ thứ Hai tới hiện tại; chỉ báo so sánh hiện chỉ có ở `Hôm nay` (so với hôm qua). Thẻ đổi nhãn và số theo kỳ đã chọn nhưng giữ cùng chiều cao giữa các kỳ: hàng tiêu đề dành sẵn chỗ cho chỉ báo so sánh, kể cả khi không có kỳ đối chiếu hợp lệ; trên màn hẹp, chỉ báo xuống hàng riêng ở cả ba trạng thái. Thẻ hiện rõ doanh thu từ đơn đã chốt trong kỳ, số đơn, và công nợ/chưa thu chỉ khi nhãn nói đúng phạm vi dữ liệu. Chạm thẻ mở `/analytics` với kỳ đã chọn; màn phân tích có diễn biến doanh thu, chi phí đã ghi, giá vốn/lãi gộp ước tính, bán chạy và đường tới danh sách đơn. Chi phí đã ghi hiển thị riêng vì chi nguyên liệu có thể trùng với giá vốn, không trừ hai lần để gọi là lãi ròng. Kết quả sau chi phí của `AC-007` còn chờ quy tắc phân loại chi nguyên liệu và giá vốn. Không trộn “doanh thu”, “đã thu” và “còn nợ” trong cùng một con số. Nếu thiếu dữ liệu, hiển thị trạng thái trống rõ nghĩa, không dùng số mẫu như dữ liệu thật. Khi số liệu của một kỳ đang tải, thẻ hiện khối giữ chỗ cùng kích thước rồi các số hiện dần theo thứ tự; không hiện `0đ` hay trạng thái trống trước khi biết kết quả. Kỳ đã tải trước đó cập nhật ngay, số chạy từ giá trị cũ sang giá trị mới. Chế độ Giảm chuyển động của hệ điều hành chỉ giữ hiệu ứng hiện/mờ ngắn. Cơ sở yêu cầu: [`BR-004`](../product/business-requirements.md), [`FR-004`, `FR-006`, `AC-004`](../product/product-requirements.md).
...
Voice mode ưu tiên màn hình tập trung, có nút đóng rõ ràng, chỉ báo đang nghe/đang xử lý/không nghe được và waveform theo input thực. Nhánh Voice hiện đo âm lượng micro để điều khiển waveform khi được cấp quyền; dữ liệu PCM chỉ dùng tại chỗ cho hiệu ứng, không lưu hoặc gửi lên dịch vụ. Bản nháp trên Voice có một lối “Thêm món” theo trạng thái đơn; nút này ẩn khi “Sửa” mở. Trong “Sửa”, mỗi dòng có tăng/giảm số lượng và “Xóa” riêng; ngoài “Sửa” chỉ hiển thị số lượng. Câu nhập “xóa/bỏ [tên món]” vẫn xóa cả dòng; tạm tính đổi ngay. Chuyển giọng nói thành text, lập đơn/ghi chi hoặc hỏi đáp bằng audio vẫn là đích tích hợp sau. Output khi tích hợp phải lên **bản nháp để sửa/xác nhận**, không tự ghi sổ; nếu AI lỗi hoặc timeout, giữ input và cho chuyển sang text/POS. Đây là hướng tương tác dựa trên [`FR-003`, `FR-008`, `FR-020`, `FR-021`, `NFR-006`, `AC-008`, `AC-010`, `AC-011`, `AC-024`](../product/product-requirements.md), chưa phải toàn bộ hành vi đã chạy trong app.
...
| 3. Zen ring và AI | Ring, menu, kéo và giữ đã có trong mock; menu dẫn tới màn Voice/Chat demo hiện hữu. Nhánh Voice bổ sung waveform theo âm lượng micro; chuyển lời nói thành text và AI đa năng thuộc giai đoạn tích hợp sau. | Kiểm tra ring trên các tab, safe area, quyền micro và trạng thái không có tín hiệu; không nhận câu gợi ý mock là bản ghi âm thật. |
...
**Chưa kiểm chứng trên thiết bị:** mức màu/shadow và font scale trên iOS/Android; quyền microphone và waveform theo âm lượng trên iOS/Android. Prototype HTML chỉ chứng minh ý tưởng bố cục/tương tác, không chứng minh các tích hợp đó.
