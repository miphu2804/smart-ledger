...
export function FakeQR({ seed, size = 160 }: { seed: string; size?: number }) {
  ...
  for (let y = 0; y < n; y++) {
    for (let x = 0; x < n; x++) {
      ...
      if (rand() < 0.48) {
        cells.push(<Rect key={`${x}-${y}`} x={x * cell} y={y * cell} width={cell} height={cell} fill="#1B2B40" />);
      }
    }
  }
  const eye = (x: number, y: number) => (
    <React.Fragment key={`e${x}${y}`}>
      <Rect x={x * cell} y={y * cell} width={7 * cell} height={7 * cell} fill="#1B2B40" rx={cell} />
      <Rect x={(x + 1) * cell} y={(y + 1) * cell} width={5 * cell} height={5 * cell} fill="#fff" rx={cell * 0.6} />
      <Rect x={(x + 2) * cell} y={(y + 2) * cell} width={3 * cell} height={3 * cell} fill="#2858D8" rx={cell * 0.5} />
    </React.Fragment>
  );
  return (
